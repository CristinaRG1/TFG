...
myTree::myTree(){
  ...
  tree->Branch("mtll",&mt_ll,"mtll/F");
  tree->Branch("DeltaPhill",&DeltaPhi_ll,"DeltaPhill/F");  
  tree->Branch("DeltaPhil2n",&DeltaPhi_l2n,"DeltaPhil2n/F");  
  tree->Branch("mtw2",&mt_l2n,"mtw2/F");
...
if (!skipEvent && v_tlv_all_leptons.size() == 2) {
    if (abs(v_tlv_all_leptons[0].GetPdgId() + v_tlv_all_leptons[1].GetPdgId()) == 2) { // OSDF WW region
        ...
        double mt_ll = sqrt(2 * ptl1_ * ptl2_ * (1 - cos(DeltaPhi_ll)));
        double mt_l2n = sqrt(2 * ptl2_ * ptmiss * (1 - cos(DeltaPhi_l2n)));
        
        if (ptl1_ > 25.0 && ptl2_ > 20.0 && ptll_ > 30.0 && ptmiss > 20.0 && mll > 20.0 && abs(DeltaPhi_ll) > 2.0 && mt_ll > 60.0 && mt_l2n > 30.0) {
            wwRegionPass = true;
        }
        
        if (!wwRegionPass) skipEvent = true;
    }
}
  ...
  // ---------------------------------------------------------------------------------------  
    ...
    if (v_tlv_all_leptons.size()>1) {
      ...
      // New Variables Defined
      ptll_ = (v_tlv_all_leptons[0].GetP4() + v_tlv_all_leptons[1].GetP4()).Pt();
      mll = (v_tlv_all_leptons[0].GetP4() + v_tlv_all_leptons[1].GetP4()).M();
      DeltaPhi_ll = TVector2::Phi_mpi_pi(v_tlv_all_leptons[0].GetP4().Phi() - v_tlv_all_leptons[1].GetP4().Phi());
      mt_ll = sqrt(2 * ptl1_ * ptl2_ * (1 - cos(DeltaPhi_ll)));
    ...
    if (v_tlv_all_neutrinos.size()>1) {
      ...
      // New Variables Defined
      ptmiss = (v_tlv_all_neutrinos[0].GetP4() + v_tlv_all_neutrinos[1].GetP4()).Pt();
      
    }
    
    
    if (v_tlv_all_leptons.size()>1 && v_tlv_all_neutrinos.size()>1 ) { 
        TLorentzVector neutrinos = v_tlv_all_neutrinos[0].GetP4() + v_tlv_all_neutrinos[1].GetP4();
        DeltaPhi_l2n = TVector2::Phi_mpi_pi(v_tlv_all_leptons.at(1).GetP4().Phi() - neutrinos.Phi());
        mt_l2n = sqrt(2 * ptl2_ * ptmiss * (1 - cos(DeltaPhi_l2n)));
    }
